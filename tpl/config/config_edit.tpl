...
<form action="?mod=extra-config&plugin=xfields&action=doedit&section={{ sectionID }}" method="post" name="xfieldsform">
	...
	<div class="panel panel-default">
		...
		{% if (not flags.editMode) %}{{ lang.xfconfig['title_add'] }}{% else %}{{ lang.xfconfig['title_edit'] }} ({{ id }}){% endif %}
			<div class="panel-head-right custom-switch">
		...
		<table class="table table-sm">
			...
			<tbody id="type_images" class="bg-light">
				...
				<tr>
					...
					<td width="50%">
						<input id="images_imgThumb" type="checkbox" name="images_imgThumb" value="1" {{ images.imgThumb }} />
				...
				<tr>
					...
					<td width="50%">
						<div class="input-group">
							<input type="number" name="images_thumbWidth" value="{{ images.thumbWidth }}" class="form-control" style="max-width:100px; text-align: center;"/>
							<div class="input-group-prepend input-group-append">
								<label class="input-group-text">x</label>
							</div>
							<input type="number" name="images_thumbHeight" value="{{ images.thumbHeight }}" class="form-control" style="max-width:100px; text-align: center;"/>
						</div>
					</td>
				</tr>
				<tr>
			...
			<tbody>
				<tr>
					...
					<td width="50%">
						<div class="input-group">
							...
							<div class="input-group-append">
								<a class="btn btn-outline-primary" data-toggle="popover" data-placement="left" data-trigger="focus" data-html="true" data-content="{{ lang.xfconfig['storage#descr'] }}" tabindex="0">
									<i class="fa fa-question"></i>
								</a>
							</div>
						</div>
				...
				<tbody id="storageRow" class="{{ not(storage) ? 'd-none' : '' }}">
				<tr>
					...
					<td width="50%">
						<div class="input-group">
							<select id="db.type" name="db_type" class="custom-select">
								...
								<!-- <option value="int" {{ 'int' == db_type ? 'selected' : '' }}>int - только цифры</option> -->
				...
				<tr id="type_url" class="bg-light">
					<td colspan="2">Поддержка полей как перекрестные гиперссылки
						<small class="form-text text-muted">ВНИМАНИЕ!<br>Для использование значений в доп поле как гиперссылку нужно, чтобы был выбран:<br><b>Тип поля</b> - Текстовый<br><b>Режим сохранения данных</b> - Персональное поле БД</small>
					</td>
					<td width="50%"><input type="checkbox" name="text_xf_field" value="1" {{ xf_field }} /><i class="fa fa-question-circle position-left" data-toggle="popover" data-placement="top" data-trigger="focus" tabindex="0" data-content="При активации значения из доп. поля будут выводится в виде ссылки, которая будет группировть новости связанные с этим значениям с другими публикациями. Также допускается перечисление значений в доп. поле через запятую, при этом каждое слово будет иметь отдельную ссылку." data-original-title="Гиперссылка" title="Гиперссылка" style="padding-left: 10px;vertical-align: middle;cursor: pointer;"></i></td>
				</tr>
				</tbody>
				<tbody>
				<tr>
